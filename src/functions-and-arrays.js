...
if (longestWord.length < arrWords[i].length) {
  ...
  return longestWord;
//} else if (longestWord.length == longestWord.length) {
...
function averageNumbers(avgNum) {
  if (avgNum.length == 0){
    ...
    for (let i=0; i<avgNum.length; i++){
      return (counter += avgNum[i]) / avgNum.length;
    }
    }
...
function averageWordLength(word) {
  if (word.length == 0){
    ...
    for (let i=0; i<word.length; i++){
      return (counter += word[i].length) / word.length;
    }
 }
}
...
if (updatedArray.indexOf(word) === -1) {
