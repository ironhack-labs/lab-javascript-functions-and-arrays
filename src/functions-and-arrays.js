...
function findLongestWord(arr) {
  ...
  for (let i of arr) {
    if (i.length > current.length) {
      current = i;
    } else if (i.length === current.length) {
      continue;
    } else {
      continue;
    }
...
// Iteration #3.1 Bonus: //PENDIENTE THROW ERROR WHEN UNSUPPORTED DATA TYPE
