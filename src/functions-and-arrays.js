...
function findLongestWord( arrayOfAnyWords ) {
  if ( arrayOfAnyWords.length === 0 ) {
    return null;
  }
  let longestWord = "";
  for ( let i = 0; i < arrayOfAnyWords.length; i++ ) {
    if ( arrayOfAnyWords[i].length > longestWord.length ) {
      longestWord = arrayOfAnyWords[i];
    }
  }
  return longestWord;
}
