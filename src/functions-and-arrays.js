...
function filterOut(original, toRemove) {
  ...
  const filteredArr = original.filter((str) => !toRemove.includes(str));
...
function greatestProduct(matrix) {
  ...
  // Helper function to calculate the product of four numbers
  ...
  for (let i = 0; i < rows; i++) {
    for (let j = 0; j <= columns - 4; j++) {
      const product = calculateProduct(matrix[i][j], matrix[i][j + 1], matrix[i][j + 2], matrix[i][j + 3]);
