...
function sum(mixedArr) {
  let result = 0;
  mixedArr.forEach(function (amount) {
...
function uniquifyArray(wordsUnique) {
  ...
  for (let i = 0; i < wordsUnique.length; i++) { 
    const info = wordsUnique[i];

    if (singularWords.indexOf(info) === -1) { 
      singularWords.push(info); 
    }
  }
...
function doesWordExist(wordsFind, info) {
...
function howManyTimes(wordsCount, wordToSearch) {
  let count = 0;
  wordsCount.forEach(function (info) {
