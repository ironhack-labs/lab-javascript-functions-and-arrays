...
function uniquifyArray(words) {
  if (words.length === 0) {
    ...
    for (l = 0; l < words.length; l++) {
      if (!finalArray.includes(words[l])) {
        finalArray.push(words[l]);
      }
    }
    return finalArray;
  }
...
function doesWordExist(words, findedWord) {
  if (words.length === 0) {
    return null;
  } else if (words.includes(findedWord) === true) {
    return true;
  } else if (words.includes(findedWord) === false) {
    return false;
  }
}
...
function howManyTimes(wordsCount, countedWord) {
  let counter = 0;
  for (m = 0; m < wordsCount.length; m++) {
    if (wordsCount[m] === countedWord) {
      counter++;
    }
  }
  return counter;
