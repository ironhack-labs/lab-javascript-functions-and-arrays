    ...
    if (mixedArr.length > 0) {
        for (let i = 0; i < mixedArr.length; i++) {
            if (typeof mixedArr[i] === "number") {
                ...
                let lengthOfString = mixedArr[i].length;
                sum += lengthOfString;
            } else if (typeof mixedArr[i] === "boolean") {
                let lengthOfBoolean = Number(mixedArr[i]);
                sum += lengthOfBoolean;
            } else if (
...
// should return: 5.7// Bonus - Iteration #4.1
function avg(mixedArr) {
    if (mixedArr.length > 0) {
        let sumMixedArray = sum(mixedArr);
        let average = sumMixedArray / mixedArr.length;
        return average;
    } else {
        return null;
    }
...
function uniquifyArray(wordsUnique) {
    if (wordsUnique.length > 0) {
        return wordsUnique.filter(
            (item, index) => wordsUnique.indexOf(item) === index
        );
    } else {
        return null;
    }
...
function doesWordExist(wordsFind, word) {
    if (wordsFind.length > 0) {
        let wordFind = wordsFind.includes(word);
