...
function averageWordLength(thingAver) {

    let total = 0;

    if (!thingAver.length) {
        return null;
    } else if (thingAver.length === 1) {
        return thingAver[0].length;
    } else {
        for (let i = 0; i < thingAver.length; i++) {
            total += thingAver[i].length;
        }
        return total / thingAver.length;
    }
}
...
//length 1,2,3...
function doesWordExist(wordAr, searchWord) {
...
function howManyTimes(arrayWords, wordSearch) {
    ...
    if (arrayWords.indexOf(arrayWords[i]) === -1) {
