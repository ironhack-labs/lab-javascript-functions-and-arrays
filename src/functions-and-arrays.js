...
// Iteration #3.1 Bonus:
// [6, 12, 'miami', 1, 'barca', '200', 'lisboa', 8, [], {}]
function sum(things) {
  const intArray = [];
  ...
  for (let thing of things) {
    switch (typeof thing) {
      ...
      case "string":
        intArray.push(thing.length);
    }
  }

  return things.length ? intArray.reduce((a, b) => a + b) : 0;
}
...
function averageNumbers(numbers) {
  ...
  if (numbers.length) {
    return sumatoria / numbers.length;
  } else {
    return null;
  }
}
...
function averageWordLength(words) {
  ...
  for (let word of words) {
    sumatorio = sumatorio + word.length;
  }

  if (words.length) {
    return sumatorio / words.length;
  } else {
    return null;
  }
}
...
function avg(words) {
  if (!words.length) {
    ...
    for (let num of sumatorio) {
      sum += num;
    }

    return sum / sumatorio.length;
  }
}
