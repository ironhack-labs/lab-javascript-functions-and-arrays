...
console.log(maxOfTwoNumbers(1, 16));
...
function uniquifyArray(mixedArr) {
  const uniqueArr = [];
  for (let i = 0; i < mixedArr.length; i++) {
    if (uniqueArr.indexOf(mixedArr[i]) === -1) {
      uniqueArr.push(mixedArr[i]);
    }
  }
  return uniqueArr;
}

console.log(uniquifyArray(mixedArr));
...
console.log(doesWordExist(words, 'false'));
console.log(doesWordExist(words, 'trouble'));
console.log(doesWordExist(words, 'veryfalse'));
