...
function averageNumbers(numArr) {
  if (!numArr.length) {
    //can we put also numArray.length = ''?
    return null;
  }
...
function avg(arrayOfElements) {
  ...
  for (const element of arrayOfElements) {
    if (typeof element === 'string') {
      ...
      arrayTotal += number(arrayOfElements);
    }
...
function howManyTimes(arrayOfWords, theWordToSearchFor) {
  let number = 0;

  for (let i = 0; i < arrayOfWords.length; i++) {
    if (theWordToSearchFor === arrayOfWords[i]) {
      number += 1; // number++
    }
  }
