...
function sumNumbers(sumArray) {
  if (sumArray === "undefined" || sumArray.length === 0) {
    return 0;
  } else if (sumArray.length === 1) {
    return sumArray.reduce(reducer);
  } else if (sumArray.every((item) => item === 0)) {
    return 0;
  } else {
    return sumArray.reduce(reducer);
  }
...
function averageWordLength(avgWordLgt) {
  if (avgWordLgt === "undefined" || avgWordLgt.length === 0) {
    return null;
  } else if (avgWordLgt.length === 1) {
    return avgWordLgt[0].length / avgWordLgt.length;
  } else {
    return avgWordLgt.join("").length / avgWordLgt.length;
  }
}
...
function doesWordExist(wordsArray, findElement) {
  if (wordsArray === "undefined" || wordsArray.length === 0) {
    return null;
  } else if (wordsArray.includes(findElement) === true) {
    return true;
  } else {
    return false;
  }
