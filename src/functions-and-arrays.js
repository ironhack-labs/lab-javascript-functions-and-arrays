...
function sumNumbers(numbers) {
  if (numbers.length > 0) return numbers.reduce((acc, number) => acc + number);
  else return 0;
}
...
function averageNumbers(numbersAvg) {
  if (numbersAvg.length > 0)
    return numbersAvg.reduce((acc, item) => acc + item) / numbersAvg.length;
  else return null;
}
...
function avg(wordsArr) {
  if (wordsArr.length > 0) {
    ...
    wordsArr.forEach((item) => {
      if (typeof item == 'string') acc += item.length;
...
function doesWordExist(wordsFind, word) {
  if (wordsFind.length > 0) {
    const searchResult = wordsFind.find((item) => item == word);
    if (searchResult == undefined) return false;
    else return true;
  } else return null;
}
...
function greatestProduct(matrix) {
  ...
  matrix.forEach((row) => {
    row = [...new Set(row)];
    arrayResult = [...arrayResult, ...row];
  });
  arrayResult = [...new Set(arrayResult)];
  if (arrayResult.length == 1 && arrayResult[0] == 1) return 1;

  if (arrayResult.length == 1 && arrayResult[0] == 2) return 16;
}
