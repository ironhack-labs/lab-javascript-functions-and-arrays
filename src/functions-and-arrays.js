...
for(let i = 0; i<words.length;i++){
  wordinArray = words[i]
  if(longWord<wordinArray.length){
    longWord = wordinArray.length;
    longestIndex = i;
  }
}

  const text = `The longest word is: , ${words[longestIndex]}`
  return text;
}
...
function howManyTimes(words, index){
  let repe = 0;
  for(let i = 0; i<words.length; i++){
    if(index == words[i]){
      repe++
