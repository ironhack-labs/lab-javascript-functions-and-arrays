...
if(a.length === 0) {
  return null;
   ...
   if(firstWord.length > currentLongest.length) {
     currentLongest = firstWord;
...
let currentSum = 0;
arrayNum.forEach(element => currentSum += element);
return currentSum;
}
  ...
  if (mixedArray.length === 0) {
    ...
    for(let i = 0; i< mixedArray.length; i++) {
      if(typeof mixedArray[i] === 'number') {
        currentSum += mixedArray[i];
      } else if(typeof mixedArray[i]=== 'string') {
            currentSum += mixedArray[i].length;
      } else {
        currentSum += Number(mixedArray[i]);
      }
    }
  }
  return currentSum;
...
function averageNumbers(arrayNum) {
  ...
  let arrayTotal = sumNumbers(arrayNum);
...
let arrayLength = arrayWords.length;
let numOfLetters = 0;
if( arrayWords.length === 0) return null;

for(let i = 0; i < arrayWords.length; i ++) {
  numOfLetters += arrayWords[i].length;
}

return numOfLetters/arrayLength;
...
for(let i = 0; i < arrayOfWords.length; i ++) {
  if(newUniqueArray.indexOf(arrayOfWords[i])=== -1) {
    newUniqueArray.push(arrayOfWords[i]);
  }
}
