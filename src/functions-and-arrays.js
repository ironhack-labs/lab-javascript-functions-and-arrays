...
function findLongestWord(arrays) {
  ...
  if (length === 0) {
    ...
    for (let i = 0; i < length; i++) {
      if (arrays[i].length > longestWord.length) {
        longestWord = arrays[i];
      }
    }
...
function averageWordLength(wordArr) {
  ...
  if (length === 0) {
    ...
    for (let i = 0; i < length; i++) {
      ...
      averageLength = totalLength / length;
    }
...
function avg(mixedArr) {
  ...
  for (let i = 0; i < length; i++) {
    if (typeof mixedArr[i] === "string") {
      sumLength += mixedArr[i].length;
    } else if (mixedArr[i] === true) {
      sumLength += 1;
    } else if (mixedArr[i] === false) {
      sumLength += 0;
    } else if (typeof mixedArr[i] === "number") {
      sumLength += mixedArr[i];
    } else if (Array.isArray(mixedArr[i]) || typeof mixedArr[i] === "object") {
      throw new Error("Unsupported data type");
    }
  }
...
console.log(doesWordExist(wordsFind, "eat"));
