...
function findLongestWord(arr) {
  ...
  for (i=0;i<arr.length;i++){
    wordCount.push(arr[i].length);
  }

  if (wordCount.length === 0) {
    return null
  } else {
    const positionOfHighestNum = wordCount.indexOf(Math.max.apply(null, wordCount));
    return arr[positionOfHighestNum]
  }
...
function averageNumbers(arr) {
  ...
  if (arr.length === 0) {
    ...
    const sumAverageArr = sumNumbers(arr);
    const averageOfArr = sumAverageArr / arr.length;
    return averageOfArr;
  }
...
function uniquifyArray(arrOfWords) {
  ...
  for (i = 0 ; i < arrOfWords.length; i++){
    if (arrUniqueWords.indexOf(arrOfWords[i]) === -1) {
      arrUniqueWords.push(arrOfWords[i]);
    } else {
      continue
    }
  }
...
function howManyTimes(arr,word) {
  if (arr.length === 0){
    ...
    let numberOfTimes = 0;
    for (i = 0; i < arr.length; i++) {
      if (arr.indexOf(word) !== -1 && word === arr[i]) {
...
function greatestProduct(matrix) {
  ...
  for (i = 0; i < matrix.length; i++) {
    for (j = 0; j < matrix[i].length; j++) {
        if (i-2>0) {
          arrOfProducts.push(matrix[i][j]*matrix[i-1][j]*matrix[i-2][j]*matrix[i-3][j]);
        }
        if (j-2>0) {
          arrOfProducts.push(matrix[i][j]*matrix[i][j-1]*matrix[i][j-2]*matrix[i][j-3]);
        }
      }
    }
