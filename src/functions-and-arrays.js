...
function averageWordLength(avWords) { 
  if (avWords.length === 0) {
    return null;
  }
  let total = 0;
  for (let i = 0; i < avWords.length; i++) {
    total = total + avWords[i];
  }
  return total / avWords.length;
}
...
function uniquifyArray(uniWords) {
  if (uniWords.length === 0) {
    return null;
  }
  let uniqueArray = []
  uniWords.forEach(word => {  
