...
function findLongestWord(wordsArray) {
  ...
  for (let i = 1; i < wordsArray.length; i++) {
    ...
    } else if (wordsArray[i].length === longestWord.length && wordsArray[i] !== longestWord) {
      return longestWord; 
    }
  }
  return longestWord;
...
// Bonus - Iteration #4.1
function avg(mixedElem) {
  if (mixedElem.length === 0) {
    return null;
  } 
}
...
function uniquifyArray(arrayOfWords) {
  ...
  const uniqueArray = arrayOfWords.filter((word, index) => {
    return arrayOfWords.indexOf(word) === index
  } );
  return uniqueArray;
}
