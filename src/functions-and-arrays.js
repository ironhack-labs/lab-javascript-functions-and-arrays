...
function sum(list) {
  if (list.length === 0) {
    ...
    let sum = 0
    for (let i = 0; i <= list.length - 1; i++) {
      if (typeof (list[i]) === "number" || typeof (list[i]) === "boolean") {
        sum += list[i]
...
function averageNumbers(list) {
  if (list.length === 0) {
    ...
    let average = sumNumbers(list) / list.length
    return average
  }
}
...
function averageWordLength(list) {
  if (list.length === 0) {
    ...
    const averageWord = sum / list.length
...
function howManyTimes(list, word) {
  ...
  while (copyList.includes(word)) {
    delete copyList[copyList.indexOf(word)]
    times ++
  }
  console.log(copyList, word, times)
  return times
}
