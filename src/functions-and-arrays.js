...
function uniquifyArray(arrayOfwords) {
  ...
  for (let i = 0; i < arrayOfwords.length; i++) {
    if (noRepeatedWords.indexOf(arrayOfwords[i]) < 0) {
      noRepeatedWords.push(arrayOfwords[i])     
    } 
  }
...
// console.log(doesWordExist(wordsFind, 'asdf'))
...
// console.log(howManyTimes(wordsCount, 'maer'))
...
function greatestProduct(matrix) {
  ...
  for (let i = 0; i < matrix.length; i++) {
    ...
    return greatestHorizontalProduct >= greatestVerticalProduct ? greatestHorizontalProduct : greatestVerticalProduct
  }

// console.log(greatestProduct(shortMatrix))
...
function greatestProductOfDiagonals(matrix) {
  ...
  for (let i = 0; i < matrix.length; i++) {
    for (let j = 0; j < matrix[i].length; j++) {
      if (matrix[i][j+3] && matrix[i+3]) {
        let product = matrix[i][j] * matrix[i+1][j+1] * matrix[i+2][j+2] * matrix[i+3][j+3];
        
        product > greatestDiagonalProduct ? greatestDiagonalProduct = product : greatestDiagonalProduct
      }
    }
  }
...
console.log(greatestProductOfDiagonals(shortMatrix))