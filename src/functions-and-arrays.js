...
function sum(mixedArr) {
	// no clue
	throw new Error(" Only numbers, strings, and booleans are allowed.");
}
...
function averageNumbers(numbersAvg) {
	let sum = sumNumbers(numbersAvg); // Use the sumNumbers function to calculate the sum
...
// policz litery = wordsArr[i].length;
// podziel przez ilosc wyrazow =  wordsArr.length;
...
function averageWordLength(wordsArr) {
	...
	for (const wordLength of wordsArr) {
		allWordsLength += wordLength.length;
	}
	return allWordsLength / wordsArr.length;
}
...
function uniquifyArray(wordsUnique) {
	// create empty array
	// iterate though exisiting uniqueArray
	//check if new array contains a word from uniqueArray
	// If not, add it
	let cleanedArray = [];
	...
	for (const word of wordsUnique) {
		if (cleanedArray.indexOf(word) === -1) {
...
// //     // Check if the current word is not already in uniqueArray
