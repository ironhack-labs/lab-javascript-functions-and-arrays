...
function doesWordExist(str) { 
 var unique = str.filter();
} 

doesWordExist( 
  ['machine', 'subset', 'trouble', 'starting', 'matter', 'eating', 'truth', 'disobedience']
)
...
const matrix = [
  ...
  function greatestProduct(inputArray ) {  
    ...
    let prod, ind;
    for (ind = 1; ind < inputArray; ind++) {
       if (ind === 1 || inputArray[ind - 1] * inputArray[ind] > prod) {
          prod = inputArray[ind - 1] * inputArray[ind];
       };
    };
    return prod;
