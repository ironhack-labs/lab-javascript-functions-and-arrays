...
function averageNumbers(array){
  ...
  sum = 0;
  array.forEach(num => sum+= num); return (sum / array.length);
}
...
function greatestProduct(array){
  ...
  array.forEach(element => {
    ...
    for(i=0; i < (element.length - 4); i++){

      prodH = element[i]*element[i+1]*element[i+2]*element[i+3]

      if(prodH > maxProductH){
  ...
  for(i=0; i < (array.length - 4); i++){
    ...
    for(j=0; j < array.length; j++){
      ...
      prodV = array[i][j]*array[i+1][j]*array[i+2][j]*array[i+3][j]
      
      if(prodV > maxProductV){
          maxProductV = prodV
      }
    }    
  } 
