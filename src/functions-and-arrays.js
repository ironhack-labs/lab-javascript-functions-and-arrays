...
function findLongestWord(name) {
  ...
  if(name.length === 0){
    ...
    for(let i = 0; i < name.length; i++){
      if(longName.length < name[i].length){
        longName = name[i];
      }
    }
    return longName;
...
function sumNumbers(input) {
  ...
  if (length == 0){return 0}
    ...
    let finalSum = 0;
    for (let i = 0; i < length; i++){finalSum += input[i]}
    return finalSum}
...
function avg(input) {
  if (input.length == 0){
    return null;
  } return (Math.floor(averageWordLength(input)*100)/100);
}
...
function uniquifyArray(input) {
  ...
  return input.filter((a, b) => input.indexOf(a) === b);
}
...
function doesWordExist(array, word) {
  ...
  if (array.find(i => {if (i.includes(word)){return true}})){return true}//checks if any element in the array has the word parameter and returns true if it does
  return false;
}
...
function howManyTimes(array, word) {
  ...
  let appearance = 0;
  if (array.filter(i => {if (i.includes(word)){return appearance+=1}})){return appearance}//adds 1 to the appearance everytime the word appears
  return 0;
}
...
function greatestProduct() {
  ...
  for(let i = 0; i < matrix.length; i++){
    for(let j = 0; j < matrix[i].length; j++){
      ...
      if(product > maxProduct){
        maxProduct = product;
        // console.log(`Max vertical is: ${maxProduct}, Index is ${i}, ${j}`);
      }
    }
  }
  return maxProduct;
