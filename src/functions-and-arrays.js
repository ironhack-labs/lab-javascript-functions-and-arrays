...
>>>>>>> 19830982c6a587a39f4a8a294fc135ba0a2609c5
...
// Iteration #3.1 Bonus:
const bonusNumbers = [2, 6, 9, 10, 7, 4, 1, 9, "Pedro", true];
function sum(bonusNumbers) {
  if (bonusNumbers.length === 0) return 0;
  let sumTotal = 0;
  for (let i = 0; i < bonusNumbers.length; i++) {
    if (typeof bonusNumbers[i] === "number") {
      sumTotal += bonusNumbers[i];
    } else if (typeof bonusNumbers[i] === "string") {
      sumTotal += bonusNumbers[i].length;
    } else if (typeof bonusNumbers[i] === "boolean") {
      if (bonusNumbers[i] === false) sumTotal += 0;
      else {
        sumTotal += 1;
      }
    } else if (typeof bonusNumbers[i] === "object") {
