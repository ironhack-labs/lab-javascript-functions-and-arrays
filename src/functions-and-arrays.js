...
function sum(arrayNumber) {
  if (arrayNumber.length === 0) {
    ...
    for (let i = 0; i < arrayNumber.length; i++) {
      ...
      } else if (typeof arrayNumber[i] === "object") {
      } else {
...
function averageNumbers(arrayNumber) {
  if (arrayNumber.length === 0) {
    ...
    let average = soma / arrayNumber.length;
    return average;
  }
}
...
function averageWordLength(arrWords) {
  if (arrWords.length === 0) {
    ...
    for (let i = 0; i < arrWords.length; i++) {
      soma += arrWords[i].length;
    }
    let avg = soma / arrWords.length;
    return avg;
  }
}

// Bonus - Iteration #4.1
const array = [[0, 1, 2, 3], "Ghost"];
function avg(array) {
  if (array.length === 0) {
    return null;
  }
