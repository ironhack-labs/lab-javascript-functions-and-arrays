...
let doesWordExist = (wordsFind, wordToBeFound) => {
  if (wordsFind.length == 0) {
    return null
  }
  if (wordsFind.includes(wordToBeFound) === true) {
    return true
  } else {
    return false
  }
...
let howManyTimes = (wordsCount, wordToBeFound) => {
  let cnt = 0
  for (let i = 0; i < wordsCount.length; i++) {
    if (wordToBeFound === wordsCount[i]) {
      cnt += 1
    } else {
      continue
    }
