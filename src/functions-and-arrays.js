...
function averageNumbers(anArray) {
  ...
  return average;
  // three lines above are the same as: return sumNumbers(anArray) / anArray.length
}
...
//There is a simpler solution to above iteration!
...
function uniquifyArray(anArray) {
  ...
  for (let i = 0; i < anArray.length; i++) {
    if (uniqueArray.indexOf(anArray[i]) === -1) { //uniqueArray.indexOf(anArray[i]) < 0 is the same - why??
...
// NOTE: A different, simpler solution to iteration above would be to use anArray.includes(aWord)
...
function greatestProduct(anArray) {
  if (!anArray?.length) {
    return null;
  }
