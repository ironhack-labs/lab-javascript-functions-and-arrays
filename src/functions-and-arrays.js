...
function sum(arrNum) {
	...
	arrNum.forEach(function (num) {
		if (typeof num === "number") {
			...
			sum += 1
		} else if (typeof num === "boolean" && num === false) {
			sum
		} else {
...
function averageWordLength(arrString) {
	...
	return letters / arrString.length
}
...
function doesWordExist(arrToSearch, word) {
	if (arrToSearch.length === 0) {
		return null
	}

	return arrToSearch.includes(word)
}
...
function howManyTimes(arrToSearch, word) {
	if (arrToSearch.length === 0) {
		return 0
	}

	let count = 0
	arrToSearch.forEach(function (str) {
		if (str === word) {
			count += 1
		}
	})
