    ...
    if (mixedArr.length) {
        mixedArr.forEach(element => {
            ...
            if (typeof element == 'string') {
                newArrayNumber.push(element.length)
            } else if (typeof element == 'boolean') {
                if (element) { newArrayNumber.push(1) } else { newArrayNumber.push(0) }
        ...
        total = sumNumbers(newArrayNumber)
...
function averageNumbers(numbersAvg) {
    //console.log("Average numbers", numbersAvg)
    let total, average = null;
    if (numbersAvg.length) {
        total = sumNumbers(numbersAvg);
...
function averageWordLength(wordsArr) {
    let wordLeng = [];
    let average = null;
    if (wordsArr.length) { wordsArr.forEach(word => wordLeng.push(word.length)); }

    return averageNumbers(wordLeng)
}
console.log('Level 2: Array of strings')
console.log(averageWordLength(wordsArr))

// Bonus - Iteration #4.1
const arraymixed = [6, 12, 'miami', 1, true, 'barca', '200', 'lisboa', 2, 10];

function avg(arraymixed) {
    let average = null;
    if (arraymixed.length) {
        average = ((sum(arraymixed) / arraymixed.length).toFixed(2)).parseFloat()
...
function doesWordExist(wordsFind, wordSearch) {
    let exist = null;
    if (wordsFind.length) {
        wordsFind.indexOf(wordSearch) != -1 ? exist = true : exist = false
    }
    return exist
}
