    ...
    return null
      ...
      for (i=0; i<words.length; i++){
        ...
        if(currentLength > longestWord.length){
          console.log(`Longest word has ${longestWord.length}`)
          longestWord = words[i];
        }
      }
      return longestWord
...
function sum(numbers){
  ...
  if(numbers.length==0){
    ...
    for (x=0; x<numbers.length; x++){
      if(typeof numbers[x] === 'object'){
        throw new Error("Unsupported data type sir or ma'am")
      }else if(typeof numbers[x] === 'number'){
        total += numbers[x]
      }else if(numbers[x] === true){
        total += 1
      }else if(numbers[x] === false){
        total += 0
      }else {
        total += numbers[x].length
      }
    }
    return total
  }
