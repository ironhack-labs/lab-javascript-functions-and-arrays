...
function sumNumbers(numbers) {
  ...
  let sum = 0;  // returns the sum of an array of numbers
...
function averageNumbers(numbersArray) {
  if (numbersArray.length === 0) // returns null if numbersArray is empty
    return null
...
function averageWordLength(wordsArr) {
  ...
  avgWordLength = totalWordsLength/wordsArr.length
...
function avg(wordsArr) {
  if (wordsArr.length === 0)
    ...
    for (element of wordsArr) {
      switch (typeof element) {
        case "boolean":
          if (element)
            wordsNormalized.push(1)
          else
            wordsNormalized.push(0)
          break;
        case "string":
          wordsNormalized.push(element.length)
          break;
          case "number":
            wordsNormalized.push(element)
            break;
        default:
          wordsNormalized.push(element)
      }
  ...
  let avg = 0
  let totalSum = 0
  for (element of wordsNormalized) {
    totalSum += element
  }
  avg = totalSum/wordsNormalized.length
...
function doesWordExist(words, wordToSearchFor) {
  if (words.length === 0){
    return null
  }

  if (words.includes(wordToSearchFor))
    return true
  else
    return false
}
