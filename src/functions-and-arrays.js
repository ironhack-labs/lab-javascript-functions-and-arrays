...
function uniquifyArray(anArr) {
  return anArr.filter((item,
      index) => anArr.indexOf(item) === index);
}


// Iteration #6: Find elements
function doesWordExist(anArr1, word1) {

  for (i=0; i <anArr1.length; i++) {
    if (anArr1[i]=== word1) {
      return true;
    }
    else{
...
function howManyTimes(anotherArr, repWord) {
  let counter=0;
  for (i=0;i<anotherArr.length; i++) {
if (anotherArr[i]===repWord) {
  counter++;
}
return counter;
  }
