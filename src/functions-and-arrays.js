...
function sum(mixedArr) {
    ...
    for (let i = 0; i < mixedArr.length; i++) {
        if (typeof mixedArr[i] === "string") {
            editedArr += mixedArr[i].length;
        } else if (typeof mixedArr[i] === "number") {
            editedArr += mixedArr[i];
...
function averageNumbers(numbersAvg) {
    if (numbersAvg.length === 0 || !Array.isArray(numbersAvg)) {
        return null;
    }
    let sumFunc = sumNumbers(numbersAvg);
    let divided = sumFunc / numbersAvg.length;
    return divided;
}
...
function doesWordExist(wordsFind, isExist) {
    if (!Array.isArray(wordsFind) || wordsFind.length === 0) {
        return null;
    }
    return wordsFind.includes(isExist) ? true : false;
}
...
function howManyTimes(wordsCount, searchArg) {
