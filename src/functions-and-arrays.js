...
function uniquifyArray(wordsNotUnique) {
  if (!wordsNotUnique.length) return null;
  let uniqueArray = [];
for (let i=0; i< wordsNotUnique.length; i++)
  if (!uniqueArray.includes(wordsNotUnique[i])) {
  uniqueArray.push(wordsNotUnique[i]) 
} return uniqueArray;
}
