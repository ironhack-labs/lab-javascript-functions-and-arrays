...
function sum(arr) {
  ...
  for (const iterator of arr) {
    if (typeof iterator === 'string') {
      ...
      sum += iterator;
    } else if (typeof iterator === 'object' || typeof iterator === 'array') {
...
function avg(arr) {
  ...
  let media = (sum / arr.length).toFixed(2);
  return Number(media);
}
...
function greatestProduct(matrix) {
  ...
  for (let i = 0; i < matrix.length - 1; i++) {
    for (let e = 0; e < matrix[i].length - 1; e++) {
      if (
        matrix[i][e] === undefined ||
        matrix[i][e + 1] === undefined ||
        matrix[i + 1][e] === undefined ||
        matrix[i + 1][e + 1] === undefined
      ) {
        continue;
      }
