...
function uniquifyArray(array) {
  ...
  // oder, wenn schon vorhanden, skippen
  let newArray = [];
  if (!array.length) return null;
  for (i of array) {
...
function greatestProduct(input) {
  ...
  for (let i = 0; i < input.length; i++) {
    for (let j = 0; j < input.length - 3; j++) {
      resultX = multiplyingX(input, j, i);
      resultY = multiplyingY(input, i, j);
      if (resultX > product) {
        product = resultX;
      } else if (resultY > product) {
        product = resultY;
      }
    }
  }
...
// This is working D1/D2
function greatestProductOfDiagonals(input) {
  let product = 0;
  let resultD1, resultD2;
  for (let i = 0; i < input.length - 3; i++) {
    for (let j = 0; j < input.length - 3; j++) {
      resultD1 = multiplyingD1(input, j, i);
      resultD2 = multiplyingD2(input, j, i);
      if (resultD1 > product) {
        product = resultD1;
      } else if (resultD2 > product) {
        product = resultD2;
      }
    }
  }
...
function multiplyingX(arr, a, b) {
  return arr[a][b] * arr[a + 1][b] * arr[a + 2][b] * arr[a + 3][b];
}

function multiplyingD1(arr, a, b) {
  return arr[a][b] * arr[a + 1][b + 1] * arr[a + 2][b + 2] * arr[a + 3][b + 3];
}

function multiplyingD2(arr, a, b) {
  return arr[a][b + 3] * arr[a + 1][b + 2] * arr[a + 2][b + 1] * arr[a + 3][b];
}

// Extra Bonus - Matrix Generator
function makeMatrix(width, height, lowestNumber, highestNumber) {
  let createdMatrix = [];
  for (let i = 0; i < width; i++) {
    createdMatrix[i] = [];
    for (let j = 0; j < height; j++) {
      createdMatrix[i][j] = getRandomNumber(lowestNumber, highestNumber);
    }
  }
