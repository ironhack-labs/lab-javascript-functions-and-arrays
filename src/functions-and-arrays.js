...
function averageNumbers(numArr) {
  ...
  if (!numArr.length) {
    ...
    let totalNums = numArr.length
    let result = sum / totalNums
...
function averageWordLength(arrStr) {
  ...
  if (!arrStr.length) {
    ...
    for (i = 0; i < arrStr.length; i++) {
      let number = arrStr[i].length
      numArr.push(number)
    }
    console.log(numArr)
    let result = averageNumbers(numArr)
