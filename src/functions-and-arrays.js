...
function sumNumbers(changed) {
	return changed.reduce((acc, curr) => acc + curr, 0);
}
...
function averageWordLength(average) {
	...
	let lengths = average.map((word) => word.length);

	let sum = lengths.reduce((acc, curr) => acc + curr, 0);
	return sum / average.length;
}
