...
function averageWordLength(arrayOfNumber) {
  let wordLength = wordsArr[i].length;
  let amountOfStrings = wordsArr.length
...
function uniquifyArray(arrayOfNumber) {
  if (uniquifyArray.length === 0) {
    return 'null'};
  let newArray = wordsUnique.filter(function(element, index){
...
function doesWordExist(word) {
    let answer = wordsFind.includes(word);
