...
//   let maximum = "";
//   for (let i = 0; i < words.length; i++) {
//     if (words[i].length > maximum[i].length) {
//       maximum = words[i];
//     }
//   }
//   return words;
...
// Iteration #3.1 Bonus:
function sum(array) {
  if (array.length === 1) return numbers[0];
...
// if (!wordsArr.length) {
//   return null;
// } else if(wordsArr.length===1){return sum+=wordsArr
// }else{
//   let sum = 0;
//   let arr = [];
//   for (let i = 0; i < wordsArr.length; i++) {
...
function doesWordExist(wordFind, searchForWord) {
  if (!wordFind.length) {
    return null;
  }
  return wordFind.includes(searchForWord);
}
