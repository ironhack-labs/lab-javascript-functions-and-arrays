...
function sum(mixedArr) {
	...
	mixedArr.forEach(function(el, index) {
		if (typeof el === 'string') {
			mixedArr.splice(index, 1, el.length);
		} else if (typeof el === 'boolean') {
			mixedArr.splice(index, 1, Number(el));
		} else if (el === 'number') {
			return el;
		}
		// } else {
...
function averageWordLength(wordsArr) {
	...
	var newArr = wordsArr.map((string) => string.length);
...
function uniquifyArray(wordsUnique) {
	...
	var uniq = wordsUnique.reduce(function(word, str) {
		if (word.indexOf(str) < 0) word.push(str);
		return word;
	}, []);
	return uniq;
}
...
function howManyTimes(arr, word) {
	...
	return arr.filter((x) => x === word).length;
}
...
function greatestProduct(myBigArray) {
	myBigArray.forEach(function(element) {
		let result = element.filter((x) => x === 1);
		if (result.length === element.length) {
			newArray.push(1);
		}
	});
	if (newArray.length === matrix.length) {
		return 1;
	}
}
