...
function averageWordLength(avgWordsArr) {
    ...
    if (avgWordsArr.length === 0) {
      ...
      for (let i = 0; i < avgWordsArr.length; i++) {
        sum += avgWordsArr[i].length;
        average = sum / avgWordsArr.length;
      }
      return average
    }
...
function uniquifyArray(arrUnique) {
  ...
  for (let i=0; i<arrUnique.length; i++){
    if(arrUnique.indexOf(arrUnique[i]) === i){
      uniqueArray.push(arrUnique[i])
    }
  }return uniqueArray
}
...
function doesWordExist(wordsArr, wordToFind) {
  ...
  else if (wordsArr.includes(wordToFind)) {
    return true
  }
  else { return false }
}
