...
function sum(junk) {
  ...
  for (let i = 0; i < junk.length; i++) {
    if (typeof junk[i] === "object") {
      throw new Error("Unsupported data type sir or ma'am");
    }

    if (typeof junk[i] === "string") {
      sums += junk[i].length;
    }
    if (typeof junk[i] === "boolean") {
      if (junk[i]) {
        sums += 1;
      }
    }

    if (typeof junk[i] === "number") {
      sums += junk[i];
    }
  }
...
function uniquifyArray(arrayofString) {
  if (arrayofString.length === 0) {
    return null;
  }
  ...
  for (let i = 0; i < arrayofString.length; i++) {
    if (!newArray.includes(arrayofString[i])) {
...
function doesWordExist(stringArray, word) {
  ...
  if (stringArray.length === 1 && stringArray[0] === word) {
    return true;
  }
...
function howManyTimes(ArrayofString, word) {
  let totalCount = 0;
