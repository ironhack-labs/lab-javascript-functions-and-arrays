...
// Iteration #3.1 Bonus:
function transforToNumbers(array) {
  ...
  array.forEach(function (el) {
    switch (typeof el) {
      case "boolean":
        if (el == true) {
          transformedArray[transformedArray.length] = 1;
        } else {
          transformedArray[transformedArray.length] = 0;
        }
        break;
      case "string":
        transformedArray[transformedArray.length] = el.length;
        break;
      case "number":
        transformedArray[transformedArray.length] = el;
        break;
      default:
...
function sum(array) {
  ...
  return sumNumbers(transforToNumbers(array));
}
...
// Bonus - Iteration #4.1
function avg(arrayOfMixed) {
  if (arrayOfMixed.length == 0) {
    return null;
  }
  let result = sum(arrayOfMixed);
  if (sum == 0) {
    return 0;
  }

  return result / arrayOfMixed.length;
}
