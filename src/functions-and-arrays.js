...
// Iteration #3.1 Bonus:    DOES NOT WORK
...
function averageNumbers(arraOfNumbers) {
  if (arraOfNumbers.length === 0){
    return null
  } else { 
  let sum = 0
  arraOfNumbers.forEach(number =>{
    sum += number
  })
  return sum / arraOfNumbers.length
}
}
...
function avg(mixedArr) {
  ...
  return currSum/ mixedArr.length
}
