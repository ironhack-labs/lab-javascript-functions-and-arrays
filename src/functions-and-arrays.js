...
function averageNumbers(avNumero){
  if (avNumero.length=='0'){
    return null
  }
  let sum = 0
  for(let i = 0; i < avNumero.length; i++){
    sum = sum + avNumero[i];
  }
  return sum/avNumero.length
}
...
function doesWordExist (words, thisWord) {
  if (words.length == ' ') {
    return null
  }
...
function greatestProduct (theProduct) {
