...
function greatestProduct(matrix) {
  ...
  for (let i=0; i<matrix.length-3; i++){
    for (let j=0; j<matrix[i].length-3; j++){
      if(greatestNumber<(matrix[i][j]*matrix[i][j+1]*matrix[i][j+2]*matrix[i][j+3])){
        greatestNumer = matrix[i][j]*matrix[i][j+1]*matrix[i][j+2]*matrix[i][j+3];
      }
      if (greatestNumber < (matrix[i][j]*matrix[i+1][j]*matrix[i+2][j]*matrix[i+3][j])){
