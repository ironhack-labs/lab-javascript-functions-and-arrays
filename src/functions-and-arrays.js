...
function avg(array) {
  if (array.length === 0) return null;
  return Number((sum(array) / array.length).toPrecision(3));
}
...
const phone = ['iPhone', 'Samsung', 'Android', 'iOS', 'iPhone', 'Samsung', 'Nokia', 'Blackberry', 'Android'];
...
//console.log(uniquifyArray(phone));
...
function greatestProductOfDiagonals(arrays) {
  let product1;
  let product2;
  let maxProduct = 0;
  // Loop over all the elements
  for (let i = 0; i < arrays.length - 3; i++) {
    ...
    for (let j = 0; j < arrays[i].length - 3; j++) {
      for (let k = 0; k < 4; k++) {
        // To create product from the 4 elements diagonally
        // we always have to step 1 with both indexes
        product *= arrays[i + k][j + k];
      }
