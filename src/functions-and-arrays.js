...
const sum = (array) => {
  ...
  function equals(elem, index, array) {
    if (elem === 0) {
      return elem === 0;
    } else if (typeof elem === typeof elem) {
      return;
    }
  }

  if (array.every(equals)) {
    return 0;
  }
  ...
  for (let i = 0; i < array.length; i++) {
    switch (typeof array[i]) {
      case "number":
        ...
        break;
      case "string":
        onlyNumbersArray.push(array[i].length);
        break;
      case "boolean":
        if (array[i] === true) {
          onlyNumbersArray.push(1);
          break;
        } else {
          onlyNumbersArray.push(0);
          break;
        }
      default:
...
const averageWordLength = (array) => {
  ...
  return sumNumbers(lengthsArray) / lengthsArray.length;
};
...
const avg = (array) => {
  ...
  let arrayAvg = sumNumbers(newArray) / newArray.length;
  console.log(newArray);
  return Math.round(arrayAvg * 100) / 100;
};
...
const uniquifyArray = (array) => {
  ...
  for (let i = 0; i < array.length; i++) {
    if (!newUniqueArray.includes(array[i])) {
