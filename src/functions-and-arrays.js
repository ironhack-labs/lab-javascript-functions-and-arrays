...
let sumNumbers = (arr) => (arr.length > 0 ? arr.reduce((a, b) => a + b) : 0);
...
let averageNumbers = (arr) =>
  arr.length > 0 ? eval(arr.join("+")) / arr.length : null;
...
let averageWordLength = (arr) =>
  arr.length > 0 ? eval(arr.join("").length / arr.length) : null;
...
let uniquifyArray = (arr) => {
  ...
  arr.length > 0
    ? arr.forEach((e) => {
        newArr.includes(e) ? null : newArr.push(e);
      })
    : (newArr = null);
  return newArr;
};
...
let doesWordExist = (arr, word) =>
  arr.length <= 0 ? null : arr.indexOf(word) >= 0 ? true : false;
...
let greatestProduct = (matrix) => {
  ...
  for (i = 0; i <= matrix.length - 4; i++) {
    matrix.forEach((e) => {
      let sum = e[i] * e[i + 1] * e[i + 2] * e[1 + 3];
      sum > greatest ? (greatest = sum) : null;
    });
  }
