...
function maxOfTwoNumbers(wordA, wordB) {
  if (wordA >= wordB) {
    return wordA;
  } else {
    return wordB;
...
// Iteration #3: Calculate the sum of an array of numbers
...
function averageNumbers(numbersArray) {
  ...
  return sumNumbers(numbersArray) / sumElements;
}
...
function averageWordLength(wordsArray) {
  let lengthTotalWords = 0;
  if (wordsArray.length === 0) {
    return null;
  }

  for (let i = 0; i < wordsArray.length; i++) {
    lengthTotalWords += wordsArray[i].length;
  }
  let wordsAvarageLength = lengthTotalWords / wordsArray.length;

  return wordsAvarageLength;
}
