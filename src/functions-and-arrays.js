...
const mixedArr = [6, 12, 'miami', 1, true, 'barca', '200', 'lisboa', 8, 10, undefined];
...
function sum(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (typeof arr[i] === 'number') {
      ...
      throw new Error(' This function accepts only numbers, strings and booleans');
    }
  }
  return resultArray.reduce((a, b) => {
    return a + b;
  });
}
...
function averageWordLength(words) {
  ...
  for (let i = 0; i < words.length; i++) {
    wordsCount.push(words[i].length);
  }
  let wordsSum = wordsCount.reduce((a, b) => {
    return a + b;
  });
...
function avg(arr) {
  ...
  const arrayToReduce = resultArray.reduce((a, b) => {
    return a + b;
  });

  return arrayToReduce / arr.length;
}
...
function uniquifyArray(elements) {
  ...
  for (let i = 0; i < elements.length; i++) {
    if (arrayUnique.includes(elements[i])) {
      continue;
    } else {
      arrayUnique.push(elements[i]);
    }
  }
  return arrayUnique;
}
...
console.log(doesWordExist(wordsFind, 'eatindddg'));
...
function howManyTimes(words, wordToCount) {
  let wordsCounter = 0;
  for (let i = 0; i < words.length; i++) {
    if (words[i].includes(wordToCount)) {
