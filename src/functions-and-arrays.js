...
function averageWordLength(wordsArr) {
  if (wordsArr.length === 1) {
    ...
    let currentTotalArray = 0
    wordsArr.forEach((word) => {
      currentTotalArray += word.length
    })
    return currentTotalArray / wordsArr.length
  }
}
...
function uniquifyArray(wordsUnique) {
  if (wordsUnique.length === 0) {
    ...
    wordsUnique.forEach((word, index) => {
      while (wordsUnique.includes(word, index + 1)) {
        wordsUnique.splice(wordsUnique.indexOf(word, index + 1), 1)
      }
    })
  }
  return wordsUnique
