...
function filterOut(arr, toFilterOut) {
  if (arr.length === 0) return null;
  if (toFilterOut.length === 0) return arr;

  return arr.filter((element) => !toFilterOut.includes(element));
}
...
function uniquifyArray(duplicateWords) {
  ...
  const uniqueWords = duplicateWords.filter((word, index) => {
    return duplicateWords.indexOf(word) === index;
  });
