...
function howManyTimes(repeatedWords, searchWord) {
	let newArray = repeatedWords.filter((el) => {
		return searchWord === el;
	});
	// let count = 0;
	// for (let i = 0; i < repeatedWords.length; i++) {
	// 	if (searchWord=== repeatedWords[i]) count++;
	// }
	// return count;
...
function multiplyBy(numbers, multiplyer) {
	...
	numbers.forEach((element) => {
		newResult.push(element * multiplyer);
	});
...
function filterOut(original, toRemove) {
	...
	return original.filter((e) => !toRemove.includes(e));
...
function uniquifyArray(duplicateWords) {
	...
	for (let i = 0; i < duplicateWords.length; i++) {
		if (newObj[duplicateWords[i]]) {
			newObj[duplicateWords[i]] += 1;
		} else {
			newObj[duplicateWords[i]] = 1;
		}
	}
