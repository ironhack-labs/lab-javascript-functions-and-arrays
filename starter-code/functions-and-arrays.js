...
// Calculate the Average
function averageNudLongestWord (words) {
...
function findLongestWord (words) {
  ...
  words.sort(function(element1, element2) { 
...
function uniquifyArray(array){
  array.filter(function(elements, index, array) {
    if ( (array.indexOf(elements, index + 1) != -1) === false){
...
function doesWordExist (wordsArray, word) {
  return words.indexOf(wordsArray, word) ? false :  true; 
}
...
function howManyTimes (words, word) {
  ...
  return word + " appear "+ num +" of times in the array";
}
   ...
   row.forEach(function(value, colNum){
     ...
     var top;
      if ( matrix[rowNum - 1] === undefined){
       top = matrix[19][colNum];
     }
     else {
       top = matrix[rowNum - 1][colNum];
     }
     
     var bottom;
      if ( matrix[rowNum + 1] === undefined){
       bottom = matrix[0][colNum];
     }
     else {
       bottom = matrix[rowNum + 1][colNum];
     }
    ...
    if (greatNum < top*right*bottom*left){
      greatNum = top*right*bottom*left;
    }
