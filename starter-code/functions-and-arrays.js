...
function findLongestWord (wordArray) {
  ...
  var lastOfLargeArray = longestWord.length-1;
    wordArray.forEach(function(word) {
      if (word.length > longestWord[lastOfLargeArray].length) {  // If word is longer than lastof longestWord array, it will empty the array, and add the word to the array
        longestWord.splice(0, longestWord.length);
        longestWord.push(word);
      }
      else if (word.length === longestWord[lastOfLargeArray].length) { //If the word is the same length as the last word in the longestWord array, it will add it to the array (So if there are multiple "longest words" they are all accounted for)
        longestWord.push(word);
      }
    });
...
function sumArray (array) {
  var sum = 0;
    array.forEach(function(someNumber) {
      sum += someNumber;
    });
  return sum;
}
...
function doesWordExist (wordsArray, word) {
  ...
  wordsArray.forEach(function(someWord) {
    if (someWord === word) {
      doesExist = "The word is '" + word + "' is in the array.";
    }
  });
  return doesExist;
}
...
function howManyTimes (words, word) {
  var wordCount = 0;
  words.forEach(function(someWord){
    if(someWord === word) {
      wordCount += 1;
    }
  });
