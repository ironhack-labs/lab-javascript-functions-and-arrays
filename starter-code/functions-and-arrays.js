...
function averageWordLength( array ) {
	var total = array.reduce( function ( acc, val ) {
	...
	for ( var i = 0; i < matrix.length; i++ ) {
		for ( var j = 0; j < matrix.length; j++ ) {
			...
			if ( j + 3 < matrix.length ) {
				prodX *= matrix[ i ][ j + 1 ] * matrix[ i ][ j + 2 ] * matrix[ i ][ j + 3 ];
			} else if ( j + 2 < matrix.length ) {
				prodX *= matrix[ i ][ j + 1 ] * matrix[ i ][ j + 2 ];
			} else if ( j + 1 < matrix.length ) {
				prodX *= matrix[ i ][ j + 1 ];
			}
			if ( i + 3 < matrix.length ) {
				prodY *= matrix[ i + 1 ][ j ] * matrix[ i + 2 ][ j ] * matrix[ i + 3 ][ j ];
			} else if ( i + 2 < matrix.length ) {
				prodY *= matrix[ i + 1 ][ j ] * matrix[ i + 2 ][ j ];
			} else if ( i + 1 < matrix.length ) {
				prodY *= matrix[ i + 1 ][ j ];
			}
			prodArrayX[ i ][ j ] = prodX;
			prodArrayY[ i ][ j ] = prodY;
			if ( max < maxOfTwoNumbers( prodX, prodY ) ) {
				max = maxOfTwoNumbers( prodX, prodY );
			}

		}
	}
	console.log( prodArrayX );
	console.log( prodArrayY );
	return max;
}
