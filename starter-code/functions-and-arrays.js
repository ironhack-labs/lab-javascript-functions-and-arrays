...
var numbersAverage = [2, 6, 9, 10, 7, 4, 1, 9];
console.log(sumArray(numbersAverage) / numbersAverage.length);
...
function uniquifyArray(array) {
    ...
    sorted.forEach(function(element, index) {
        if (element === sorted[index - 1]) {
            console.log(sorted[index] + " is repeated!");
            sorted.splice(sorted.indexOf(element), 1);
        }
    });
    console.log("New array with no repetitions: ");
    return sorted;
}
...
// Finding Elements
function doesWordExist(words, toFind) {
    return words.includes(toFind);
}
...
function wordRepetitions(words2, toFind) {
    var counter = 0;
    words2.forEach(function(element, index) {
        if (element === toFind) {
            counter++;
        } else {
...
console.log(wordRepetitions(words2, "matter"));
