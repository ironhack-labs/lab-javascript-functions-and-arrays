...
function doesWordExist(arrayWords, anyWord) {
  if (arrayWords.length == 0) {
    return false;
  } else if (arrayWords.length == 1) {
    return true;
  } else if (!arrayWords.includes(anyWord)) {
    return false;
  } else {
    return true;
  }
...
function howManyTimes(arrayWords, anyWord) {
  var count = 0;
  if (arrayWords.length == 0) {
    return false;
  } else {
...
function greatestProduct(arrayMatrix) {
  arrayMatrix.forEach(function(element) {
