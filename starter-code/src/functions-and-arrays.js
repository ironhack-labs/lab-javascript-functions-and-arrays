...
function averageNumbers(inputArray) {
  if (inputArray.length === 0) {
    ...
    var averageNumber;
    averageNumber = sumArray(inputArray) / inputArray.length;
...
function doesWordExist(inputArray, word) {
  var present = false;
...
function greatestProduct (array) {
  ...
  for (var i=0 ; i< dim1; i++) {
    for (var j =0 ; j < dim2; j++) {
      console.log(i,j, indexFunc(i-1,dim1), dim1, dim2)
      var tmp = array[indexFunc(i-1,dim1)][j]
      tmp *= array[indexFunc(i+1,dim1)][j]
      tmp *= array[i][indexFunc(j-1,dim2)]
      tmp *= array[i][indexFunc(j+1,dim2)]
