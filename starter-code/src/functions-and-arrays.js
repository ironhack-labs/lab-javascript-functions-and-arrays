...
function sumArray(arrayOfNumbers) {
  ...
  if (arrayOfNumbers.length === 0) {
    ...
    sum = (acumulator, currentValue) => acumulator + currentValue;
...
function averageNumbers(arrayOfNumbers) {
  var reducer, sum;
  if (arrayOfNumbers.length !== 0) {
    reducer = (acumulator, currentValue) => acumulator + currentValue;
    sum = arrayOfNumbers.reduce(reducer);
...
function uniquifyArray(arrayOfWords) {
  if (arrayOfWords.length > 0) {
    var uniq = arrayOfWords.reduce(function(a, b) {
      if (a.indexOf(b) < 0) {
        a.push(b);
      }
      return a;
...
function doesWordExist(arrayOfWords, word) {
  var found;
  if (arrayOfWords.length > 0) {
    found = arrayOfWords.indexOf(word);
    if (found >= 0) {
      return true;
    } else {
      return false;
