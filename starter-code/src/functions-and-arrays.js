...
function averageWordLength(wordsArr) {
  ...
  for (let i = 0; i < wordsArr.length; i++) {
    newNumberArray.push(wordsArr[i].length);
  }
  // return averageNumbers(newNumberArray)
  for (let i = 0; i < newNumberArray.length; i++) {
    sum = sum + newNumberArray[i];
  }
  var res = sum / wordsArr.length;
  return res;
}
...
function uniquifyArray(wordsUnique) {
  ...
  var arrSameWord = wordsUnique.filter(
