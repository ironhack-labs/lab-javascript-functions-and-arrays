...
function uniquifyArray(WordsOfArray){
  let returnArray = [];
  WordsOfArray.forEach(element => {
...
function howManyTimes(WordsOfArrays, searchString){
  let count = 0;
  WordsOfArrays.forEach(element => {
...
function greatestProduct(matrix) {
  let product = matrix[0][0] * matrix[0][1] * matrix[0][2] * matrix[0][3];
  for (j = 0; j < 20; j++) {
    for (let i = 0; i <= 15; i++) {
      if (
        product <
