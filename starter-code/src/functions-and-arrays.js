...
function findLongestWord(arrayOfWords) {
  ...
  for (let i = 0; i < arrayOfWords.length; i++) {
    if (arrayOfWords[i].length > longestWordIndex) {
      longestWordIndex = arrayOfWords[i].length;
      longestWordToReturn = arrayOfWords[i];
    }
  }
  return longestWordToReturn;
}

//findLongestWord(["a", "zab", "12abc", "$$abcd", "abcde", "ironhack"]);
...
function averageNumbers(arrayOfElements) {
  if (Array.isArray(arrayOfElements) && arrayOfElements.length === 0) {
    return null;
  }
  return sumArray(arrayOfElements) / arrayOfElements.length;
}
averageNumbers(numbersAvg);

function averageWordLength(arrayOfElements) {
  var numberTotalOfchar = 0;
  if (Array.isArray(arrayOfElements) && arrayOfElements.length === 0) {
    return null;
  }

  for (let i = 0; i < arrayOfElements.length; i++) {
    numberTotalOfchar += arrayOfElements[i].length;
  }
  console.log(numberTotalOfchar / arrayOfElements.length);
  return numberTotalOfchar / arrayOfElements.length;
}
...
function uniquifyArray(arrayOfElements) {
  ...
  for (let i = 0; i < arrayOfElements.length; i++) {
    returnValue = arrayOfElements.indexOf(arrayOfElements[i]);
    if (returnValue >= 0) {
      let alreadyExist = uniqueList.indexOf(arrayOfElements[i]);
      if (alreadyExist === -1) {
...
//uniquifyArray(["Ironhack", "Ironhack", "Ironhack"]);
...
function doesWordExist(arrayOfWords, word) {
  if (Array.isArray(arrayOfWords) && arrayOfWords.length === 0) return false;
  for (let i = 0; i < arrayOfWords.length; i++) {
    returnValue = arrayOfWords.indexOf(word);
    if (returnValue >= 0) return true;
    else if (returnValue === -1) return false;
  }
}
...
function howManyTimes(arrayOfWords, word) {
  var count = 0;
  if (Array.isArray(arrayOfWords) && arrayOfWords.length === 0) return 0;
  for (let i = 0; i < arrayOfWords.length; i++) {
    if (doesWordExist(arrayOfWords[i], word)) count++;
  }
