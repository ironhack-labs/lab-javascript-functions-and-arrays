...
function findLongestWord(array) {
	if (array.length < 1) {
		...
		array.forEach(function (oneName) {
			if (oneName.length > longuestWord.length) {
				longuestWord = oneName;
			}
		}); // for each end
		return longuestWord;
	}
...
function averageWordLength(array) {
	...
	if (array.length < 1) {
		...
		array.forEach(function (charCount) {
	...
	if (array.length < 1) {
		...
		array.forEach(function (element) {
			...
			for (var i = 0; i < array.length; i++) {
				if (element.includes(array[i]) === true) {
					occurences++;
				}
			}
