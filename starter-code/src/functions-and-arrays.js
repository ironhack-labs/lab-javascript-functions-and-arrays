...
function findLongestWord(wordsList) {
  ...
  wordsList.forEach((word) => {
    word.length > temp.length ? (temp = word) : false;
  });
...
function averageWordLength(wordsList) {
  ...
  return characterSum / wordsList.length;
}
...
function doesWordExist(wordsList, searchterm) {
  return wordsList.includes(searchterm);
}
...
function howManyTimes(wordsList, searchterm) {
  if (!wordsList.length) return 0;

  let wordCount = 0;

  wordsList.forEach((element) => {
    element === searchterm ? wordCount++ : false;
  });
...
function greatestProduct(inputMatrix) {
  ...
  inputMatrix.map((element) => {
    greatestNumbers.push(
      element.sort((a, b) => {
        b - a;
      })[0],
    );
  ...
  greatestNumbers.sort((a, b) => {
    b - a;
  });

  for (let i = 0; i < 4; i++) {
    greatestProduct *= greatestNumbers[i];
  }
