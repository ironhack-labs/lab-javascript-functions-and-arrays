...
function averageWordLength(wordArray) {
  if (wordArray.length == 0) {
    ...
    return Math.round(addition / wordArray.length);
  }
}
...
function uniquifyArray(arrayOfWords) {
  ...
  arrayOfWords.forEach(function(element) {
    if (uniqueWordsArray.indexOf(element) < 0) {
      uniqueWordsArray.push(element);
    }
  });
...
function doesWordExist(wordTab, word) {
  let result = false;
  wordTab.forEach(function(element) {
    if (result === false) {
      result = element === word;
    }
  });
...
function howManyTimes(wordTab, word) {
  let counter = 0;
  wordTab.forEach(function(element) {
    if (element === word) {
      counter += 1;
