...
var average;
for(var i = 0; i < arr.length; i++) {
  charecter = charecter + arr[i].length;
  average = (charecter/ arr.length);
}
  return average;
 }
...
function doesWordExist(arr, arr1) {
  var emptyArray = [];
   if(arr.length === 0) {
    return false;
    ...
    for(var i = 0; i < arr.length; i++) {
      if(arr.length !== emptyArray) {
        return false;
      } else if(arr.length === arr) {
        return true;
      }
    }
  }
