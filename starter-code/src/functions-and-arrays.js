...
//solution 2 ways
function doesWordExist(arr, searchValue) {
...
// solution 2 ways
function howManyTimes(arr, searchValue) {
  let count = 0;
  arr.forEach(word => {
    if (word === searchValue) count++
  })
  return count
}
...
function greatestProduct(arr) {
  ...
  for (let i = 0; i < arr.length-3; i++) {
    // get each row except for the last 3 because value will be null if i continue it
    for (let j = 0; j < arr[i].length-3; j++) {
      let rowProd = arr[i][j] * arr[i][j+1] * arr[i][j+2] * arr[i][j+3] // calculates row
      let colProd = arr[i][j] * arr[i+1][j] * arr[i+2][j] * arr[i+3][j] // calculates column
  
      if (colProd > greatest) greatest = colProd
      if (rowProd > greatest) greatest = rowProd
    }
  }
