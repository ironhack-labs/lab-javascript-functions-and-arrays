...
function averageWordLength(arrOfWords) {
  ...
  for (let i = 0; i < arrOfWords.length; i++) {
    lengthArr.push(arrOfWords[i].length);
  }
...
function uniquifyArray(arrOfWords) {
  let newArray = [];
  for (let i = 0; i < arrOfWords.length; i++) {
    if (newArray.indexOf(arrOfWords[i]) === -1) {
...
function greatestProduct(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    for (let j = 0; j < arr[i].length; j++) {
      if (i >= 3) {
        var up = arr[i][j] * arr[i - 1][j] * arr[i - 2][j] * arr[i - 3][j];
        arrTemp.push(up);
      }
      if (i < arr.length - 3) {
        var down = arr[i][j] * arr[i + 1][j] * arr[i + 2][j] * arr[i + 3][j];
        arrTemp.push(down);
      }
      if (j >= 3) {
        var left = arr[i][j] * arr[i][j - 1] * arr[i][j - 2] * arr[i][j - 3];
        arrTemp.push(left);
      }
      if (j < arr[i].length - 3) {
        var right = arr[i][j] * arr[i][j + 1] * arr[i][j + 2] * arr[i][j + 3];
        arrTemp.push(right);
      }
    }
  }
