...
function averageWordLength(wordsArray) {
  ...
  for (let i = 0; i < wordsArray.length; i++) {
    avgWords += wordsArray[i].length;
  }
  if (avgWords != null) {
    return avgWords / numbers.length;
  } else {
    return null;
  }
}
...
function doesWordExist(words, wordToFind) {
  ...
  for (let i = 0; i < words.length; i++) {
    if (words[i].indexOf(wordToFind) >= 0) {
      wasFound = true;
    }
  }
  return wasFound;
...
doesWordExist(words, wordToFind);
...
function howManyTimes(arrayWords, arrayToSearch) {
  let countWords = 0;
  for (let i = 0; i < arrayWords.length; i++) {
    if (arrayToSearch.indexOf(arrayWords[i]) != -1) {
      countWords++;
    }
  }
  return countWords;
