...
console.log(maxOfTwoNumbers(30, 25));
...
function uniquifyArray (wordsUnique){
    ...
    for(var i=0; i>wordsUnique.length; i++){
            if(pivote.indexOf(wordsUnique[i])===-1){
                pivote.push(wordsUnique[i]);
            }
    }
    return pivote;
}

var resultado = uniquifyArray(wordsUnique);
...
function doesWordExist(wordsFind, wordforsearch){
  if(wordsFind.length === 0){
    return false;
  }
  
  for(var i = 0; i < wordsFind.length; i++){
    if(wordsFind[i] == wordforsearch){
