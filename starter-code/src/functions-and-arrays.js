...
function findLongestWord(w) {
  ...
  return w.reduce((a, b) => (a.length < b.length ? b : a), '');
}
...
function averageWordLength(w) {
  if (!w.length) return undefined;
  return w.reduce((a, b, i, arr) => a + b.length / arr.length, 0);
}
...
function uniquifyArray(w) {
  if (!w.length) return undefined;
  let myFilter = (value, i, self) => self.indexOf(value) === i;
  return w.filter(myFilter);
}
