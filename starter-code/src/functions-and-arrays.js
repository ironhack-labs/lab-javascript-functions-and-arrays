...
function findLongestWord(words) {
  ...
  indice = 0;
  var word1 = words[0].length;
  for (i = 1; i <= words.length - 1; i++) {
    word2 = words[i].length;
...
function averageNumbers(numbersAvg) {
  //  console.log(numbersAvg[0]);
  for (i = 0; i <= numbersAvg.length; i++) {
    if (numbersAvg[i] < 0)
      return (
        numbersAvg.reduce((total, item) => total + item) / numbersAvg.length
      );
  }
  if (numbersAvg.length == 1) return numbersAvg[0];
  if (numbersAvg.length == 0) return undefined;
  return numbersAvg.reduce((total, item) => total + item) / numbersAvg.length;
}
...
function averageWordLength(wordsArr) {
  ...
  for (i = 0; i <= wordsArr.length - 1; i++) {
    console.log("se", wordsArr[i]);

    arrnumber[i] = wordsArr[i].length;
  }

  averageString =
    arrnumber.reduce((total, item) => total + item) / arrnumber.length;
