...
function findLongestWord(arr) {
  let bigest;
    ...
    arr.forEach(element => {
      ...
      if (typeof bigest === 'undefined') {
        bigest = element ;
      } else if (element.length > bigest.length){
        bigest = element;
      }
...
function averageNumbers(n){
  ...
  const sum = sumArray(n);
  if(!isNaN(sum) && sum !== 0) { return sum/n.length; }
...
function doesWordExist(arr, str) {
  ...
  if (arr.length > 0) {
    for (const element of arr) {
      if(element === str){ check = true; break; }
    }
  } 
  return check;
}
const oneString = [];
oneString.push('str');
console.log(doesWordExist(oneString, 'str'))

function howManyTimes(arr, str) {
  if(arr.length===0){return false;}
 let qty = 0;
 arr.forEach(element => {
  console.log(element.indexOf(str));
