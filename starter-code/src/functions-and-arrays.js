...
function findLongestWord(array) {
  let maxWord = array[0];
  array.forEach(obj => {
    maxWord = obj.length > maxWord.length ? obj : maxWord;
  });
  return maxWord;
}
...
function greatestProduct(array) {
  ...
  for (let i = 0; i < matrix.length - 3; i++) {
    for (let j = 0; j < matrix[i].length - 3; j++) {
      ...
      if (xCurrentProduct > highestProduct) {
        console.log(matrix[i][j]);
        highestProduct = xCurrentProduct;
      } else if (yCurrentProduct > highestProduct) {
        highestProduct = yCurrentProduct;
      }
    }
  }
