...
function uniquifyArray(wordsUnique) {
  if (wordsUnique.length === 0) {
    ...
    for (var i = 0; i < wordsUnique.length; i++) {
      var indices = [];
      var element = wordsUnique[i];
      var position = wordsUnique.indexOf(element);
      while (position != -1) {
        indices.push(position);
        position = wordsUnique.indexOf(element, position + 1);
      }
      console.log(position);
      console.log(indices);

      if (indices.length >= 2) {
        var indicebis = indices[1];
        wordsUnique.splice(indicebis, 1);
      }
    }
  }
  return wordsUnique;
