...
function findLongestWord(words) {
  ...
  if (words.length == 1) return words[0]; // si array a 1 élément (i = 0), retourne cet élément.
...
// console.log(sumArray([10, 5, 4, 32, 8]));
  ...
  if (wordsUnique.length > 0) {
    for (let i = 0; i < wordsUnique.length; i++) {
      if (wordsUnique.indexOf(wordsUnique[i]) == i) {
        uniqueItems.push(wordsUnique[i]);
      } else {
        continue;
      }
    }
