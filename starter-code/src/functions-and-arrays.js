...
function averageNumbers(arrOfNums) {
  ...
  return (sumOfArr /= arrOfNums.length);
}
...
function averageWordLength(arrOfWords) {
  ...
  for (let i = 0; i < arrOfWords.length; i++) {
    sumOfWordLengths += arrOfWords[i].length;
  }
  return (sumOfWordLengths /= arrOfWords.length);
}
...
function uniquifyArray(arrOfWords) {
  ...
  for (i = 0; i < arrOfWords.length; i++) {
    if (uniqueArr.indexOf(arrOfWords[i]) === -1) {
