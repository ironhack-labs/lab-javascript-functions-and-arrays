...
function findLongestWord(someArray) {
  let firstWord = someArray[0];
  let largestWord = null;
  for (let i = 0; i < someArray.length; i++) {
    if (firstWord.length > someArray[i].length) {
      largestWord = firstWord;
    } else if (firstWord.length < someArray[i].length) {
      largestWord = someArray[i];
    } else {
      largestWord = firstWord;
    }
  }
  ...
  // arrLength.push({word:el,len:el.length});
  //   });
  //   arrLength.sort();
}
...
//       var arrays = [], size = 4;
//         while (gridMatrix[i].length > 0){
//             arrays.push(gridMatrix[i].splice(0, size));
//       }
//     }
//      return arrays;
//   }
